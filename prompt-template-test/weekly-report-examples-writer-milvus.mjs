...
/**
 * Get vector embedding for input text.
 */
...
/**
 * Create collection if needed and make sure it is loaded.
*/
async function ensureCollection() {
  try {
    ...
    if (!hasCollection.value) {
      ...
      const fields = collectionDetail?.schema?.fields ?? [];
      const vectorField = fields.find((field) => field.name === 'vector');
      const dimFromTypeParams = vectorField?.type_params?.find((item) => item.key === 'dim')?.value;
      const existingDim = Number(vectorField?.dim ?? dimFromTypeParams ?? 0);

      if (existingDim > 0 && existingDim !== VECTOR_DIM) {
        throw new Error(
          `Vector dim mismatch for collection "${COLLECTION_NAME}": collection=${existingDim}, script=${VECTOR_DIM}. ` +
          'Please align VECTOR_DIM/model output dimension, or drop and recreate the collection.',
        );
      }
    }
    ...
    console.error('Error while ensuring collection:', error.message);
    throw error;
  }
}

/**
 * Insert weekly report examples into Milvus.
 */
async function insertExamples() {
  try {
    ...
    console.log(`\nGenerating vectors and inserting ${EXAMPLES.length} weekly report examples...`);

    const insertData = await Promise.all(
      EXAMPLES.map(async (example, index) => {
        // Use scenario + report_snippet as the embedding source text.
    ...
    const errorCount = Array.isArray(insertResult.err_index) ? insertResult.err_index.length : 0;
    if (errorCount > 0) {
      throw new Error(
        `Milvus insert failed for ${errorCount} rows: ${insertResult?.status?.reason ?? 'unknown reason'}`,
      );
    }
    console.log(`✓ Inserted ${insertedCount} records`);
    return insertedCount;
  } catch (error) {
    console.error('Error while inserting weekly report examples:', error.message);
...
async function main() {
  try {
    ...
    console.log('Write weekly report examples into Milvus');
    console.log('='.repeat(80));
