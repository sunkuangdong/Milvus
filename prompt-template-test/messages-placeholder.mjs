...
// Demo: inject conversation history into ChatPromptTemplate via MessagesPlaceholder.
...
// 2) Define a ChatPromptTemplate that includes MessagesPlaceholder.
const chatPromptWithHistory = ChatPromptTemplate.fromMessages([
  [
    'system',
    `你是一名资深工程效率顾问，善于在多轮对话的上下文中给出具体、可执行的建议。`,
  ],
  // Use MessagesPlaceholder to carry previous conversation turns.
  new MessagesPlaceholder('history'),
  [
    'human',
    `这是用户本轮的新问题：{current_input} 请结合上面的历史对话，一并给出你的建议。`,
  ],
]);

// 3) Build mock history messages + current input.
...
console.log('Messages array with conversation history:');
