...
// Demo: auto-select length-appropriate few-shot examples with LengthBasedExampleSelector.
...
// 2) Define prompt template for one example.
...
// 3) Prepare examples with clearly different lengths.
const examples = [
  {
...
// 4) Create LengthBasedExampleSelector.
const exampleSelector = await LengthBasedExampleSelector.fromExamples(examples, {
  examplePrompt,
  // Use character length as a simple approximation (token estimate is better in production).
  maxLength: 700,
...
// 6) Demo: provide a complex requirement and let selector choose fitting examples.
